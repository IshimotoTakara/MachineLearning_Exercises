...
　畳み込みニューラルネットワークは、図のように”畳み込み”（convolution）という特殊な線形変換を、行列演算の代わりにいずれかの層で行っているフィードフォワード型のニューラルネットワークである。  
より狭義には、畳み込み層（Conv層）と、プーリング層（Pooling層）という２つのそうを交互に積み重ねたフィードフォワード型のニューラルネットワークとも定義できる。  
